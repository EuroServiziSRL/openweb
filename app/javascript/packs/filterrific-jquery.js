...
console.log("loading filterrific");

// Create global Filterrific namespace
if (typeof Filterrific === 'undefined') {
  console.log("Create global Filterrific namespace");
  var Filterrific = {};
}


console.log("Define function to submit Filterrific filter form");
...
Filterrific.submitFilterForm = function(){
  ...
  // Submit ajax request
  console.log("sending to url "+url+" data ["+form.serialize()+"]");
  $.ajax({
    url: url,
    data: form.serialize(),
    type: 'GET',
...
console.log("Embed jquery.observe_field.js to observe Filterrific filter inputs");
...
(function($) {
  ...
  $.fn.filterrific_observe_field = function(frequency, callback) {
    console.log("$.fn.filterrific_observe_field");
...
Filterrific.init = function() {
  console.log("initialize filterrific");
...
jQuery(document).on('turbolinks:load', function() {
  console.log("document turbolinks:load on filterrific");
  // Prevent double initilisation. With turbolinks 5 this function
  // will be called twice: on 'ready' and 'turbolinks:load'
  $(document).off('ready page:load')
  Filterrific.init();
});

jQuery(document).on('ready page:load', function() {
  console.log("document ready on filterrific");
  Filterrific.init();
});

$(document).ready(function(){
  console.log("filterrific document.ready");
  Filterrific.init();
});